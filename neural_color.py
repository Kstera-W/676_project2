...
def build_parser():
    ...
    parser.add_argument('--initial',
            ...
            metavar='INITIAL', required=True)
    parser.add_argument('--content_gray',
            dest='content_gray', help='content gray scale image',
            metavar='CONTENT_GRAY', required=True)
    parser.add_argument('--styles',
            dest='styles',
            nargs='+', help='one or more style images',
            metavar='STYLE', required=True)
    parser.add_argument('--styles_gray',
            dest='styles_gray',
            nargs='+', help='one or more grayscale style images',
            metavar='STYLE_GRAY', required=True)
...
def main():
    ...
    style_images_gray = [imread(style_gray) for style_gray in options.styles_gray]

    width = options.width
    if width is not None:
        new_shape = (int(math.floor(float(content_image_gray.shape[0]) /
    ...
    for i in range(len(style_images)):
        ...
        style_images[i] = cv2.resize(style_images[i], (int(style_scale * target_shape[1] / style_images[i].shape[1]*style_images[i].shape[0]), int(style_scale * target_shape[1])))
        style_images_gray[i] = cv2.resize(style_images_gray[i], (int(style_scale * target_shape[1] / style_images_gray[i].shape[1] * style_images_gray[i].shape[0]), int(style_scale * target_shape[1])))
    ...
    if initial is not None:
        initial = cv2.resize(imread(initial), (content_image_gray.shape[1],content_image_gray.shape[0]))
